...
#import "ProfitAnalyseDetailVM.h"

@implementation ProfitAnalyseDetailItemVM

@end

@interface ProfitAnalyseDetailVM ()

@property (strong, nonatomic) NSMutableArray<ProfitAnalyseDetailItemVM *> *itemList;

@end

@implementation ProfitAnalyseDetailVM

- (NSMutableArray<ProfitAnalyseDetailItemVM *> *)itemList {
	if (_itemList == nil) {
	...
	[self.sessionManager POST:@"getSalesProductReport.do" parameters:parameters progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
		...
		if ([response[@"type"] integerValue] == 1) {
			...
			for (NSDictionary *dict in listData) {
				ProfitAnalyseDetailItemVM *item = [[ProfitAnalyseDetailItemVM alloc] init];
				if (self.type == 0) {
			...
			for (ProfitAnalyseDetailItemVM *item in self.itemList) {
...
- (NSString *)jsStr {
	...
	for (ProfitAnalyseDetailItemVM *item in self.listDataSource) {
		jsStr = [jsStr stringByAppendingString:[NSString stringWithFormat:@"starList.push(new PieItem('%@', %.2f));", item.name, item.percent]];
	}
	jsStr = [jsStr stringByAppendingString:@"setPieData(starList, null, '毛利占比', '%');"];
	return jsStr;
}
