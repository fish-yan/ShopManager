...
@interface CardAnalyseFilterView () {
	UIView *view;
}
...
@implementation CardAnalyseFilterView
...
- (UIView *)loadFromNib {
	UIView *xibView = [[[UINib nibWithNibName:@"CardAnalyseFilterView" bundle:nil] instantiateWithOwner:self options:nil] firstObject];
	return xibView;
}
...
- (void)radioButtonDidTouch:(UIButton *)button {
	if (self.selected) {
...
- (void)confirmButtonDidTouch {
	...
	if (self.confirmBlock) {
		self.confirmBlock(self.startField.text, self.endField.text, self.index, self.selected);
	}
}
