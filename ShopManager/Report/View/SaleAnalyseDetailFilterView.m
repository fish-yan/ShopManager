...
@interface SaleAnalyseDetailFilterView () {
	UIView *view;
}
...
@implementation SaleAnalyseDetailFilterView
...
- (UIView *)loadFromNib {
	UIView *xibView = [[[UINib nibWithNibName:@"SaleAnalyseDetailFilterView" bundle:nil] instantiateWithOwner:self options:nil] firstObject];
	return xibView;
}
...
- (void)confirmButtonDidTouch {
	...
	if (self.confirmBlock) {
		self.confirmBlock(self.startField.text, self.endField.text, self.index, self.statisticType);
	}
}
