...
@interface UnSettleCarViewController ()<UITableViewDelegate, UITableViewDataSource>
...
@implementation UnSettleCarViewController

- (void)viewDidLoad {
    [super viewDidLoad];
	self.tableView.contentInset = UIEdgeInsetsMake(301, 0, 0, 0);
	[self.tableView addSubview:self.headerView];
	[self.tableView registerNib:[UINib nibWithNibName:@"SBCarCountTableViewCell" bundle:nil] forCellReuseIdentifier:titleCell];
	[self.tableView registerNib:[UINib nibWithNibName:@"SBWashCarItemTableViewCell" bundle:nil] forCellReuseIdentifier:carCell];
...
- (UnSettleVM *)viewModel {
	if (_viewModel == nil) {
		_viewModel = [[UnSettleVM alloc] init];
	}
	return _viewModel;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	if (indexPath.row == 0) {
		SBCarCountTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:titleCell];
		cell.selectionStyle = UITableViewCellSelectionStyleNone;
		cell.titleL.text = @"未交车台次";
		cell.countL.text = self.viewModel.num;
		return cell;
	} else if (indexPath.row <= self.viewModel.listDataSource.count) {
		SBWashCarItemTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:carCell];
		cell.selectionStyle = UITableViewCellSelectionStyleNone;
		UnSettleItemVM *item = self.viewModel.listDataSource[indexPath.row-1];
